...
public class ProcessResourceService implements IProcessResourceService {
	...
	@Override
	public String processSatelliteVisibleByPeriod(ResourceType resourceType, String request, String version)
		...
		List<SatelliteTimeCoordinate<SphericalCoordinate>> sateliteVisible = this.sateliteVisibleService
				.getSatelliteVisibleByPeriod(visibleSateliteRequeste);
		...
		String response = this.responseResourceService.getFluxSateliteVisible("satellite-visible-byperiod",
				resourceType, v, fluxInformations);

		return response;
	}

	@Override
	public String processSatelliteVisibleBySatellite(ResourceType contentType, String request, String version)
			throws TechnicalException, BusinessException {

		VisibleSateliteRequest visibleSateliteRequeste = this.requestResourceService
				.getRequestSateliteVisible(contentType, version, request);

		List<SatelliteCoordinate<SphericalCoordinate>> sateliteVisible = this.sateliteVisibleService
				.getSatelliteVisibleBySatellite(visibleSateliteRequeste);

		Map<String, Object> fluxInformations = new HashMap<String, Object>();
		fluxInformations.put("satellitesVisible", sateliteVisible);

		Version v = ApiVersionUtil.getInstance().getVersion(version);
		String response = this.responseResourceService.getFluxSateliteVisible("satellite-visible-bysatellite",
