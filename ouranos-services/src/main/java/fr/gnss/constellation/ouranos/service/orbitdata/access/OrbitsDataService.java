...
import fr.gnss.constellation.ouranos.repository.sp3.file.ISp3FileRepository;
import fr.gnss.constellation.ouranos.repository.sp3.stream.ISp3InputStreamRepository;
...
public class OrbitsDataService implements IOrbitsDataService {
    ...
    private final ISp3FileRepository sp3Dao;
    ...
    public List<SatelliteTimeCoordinate<CartesianCoordinate3D>> getDatasForPeriod(LocalDateTime start, LocalDateTime end
        ...
        for (Sp3FileName sp3FileName : allSp3FileBetweenStartEnd) {
            ...
            try {
                Sp3File sp3File = sp3Dao.getFile(sp3FileName);
                if (sp3File == null) {
                    sp3File = sp3Dao.saveSp3File(sp3FileName, sp3InputStreamRepository.downloadSp3File(sp3FileName, true));
                }
