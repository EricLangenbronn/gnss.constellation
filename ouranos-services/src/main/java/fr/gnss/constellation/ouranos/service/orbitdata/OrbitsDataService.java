...
public class OrbitsDataService implements IOrbitsDataService {
	...
	public List<SatelliteTimeCoordinate<CartesianCoordinate3D>> getDatasForPeriod(LocalDateTime start, LocalDateTime end, EphemerideType ephemerideType,
		...
		try {
			...
			String message = "Impossible d'obtenir les données pour cette période. [start=" + start + ", end=" + end + ", ephemerideType=" + ephemerideType
					+ ", orbitType=" + orbitType + "]";
			LOGGER.error(message);
			throw new TechnicalException(message);
		} catch (BusinessException e) {
			String message = "Impossible de parser les données pour cette période. [start=" + start + ", end=" + end + ", ephemerideType=" + ephemerideType
					+ ", orbitType=" + orbitType + "]";
			LOGGER.error(message);
			throw new TechnicalException(message);
		}
